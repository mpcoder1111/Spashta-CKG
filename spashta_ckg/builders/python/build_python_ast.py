...
def main():
    ...
    # Pre-populate global symbol registry from existing AST if provided
    if getattr(args, "existing_ast", None) and Path(args.existing_ast).exists():
        try:
            ...
            reparse_rel_set = {p.relative_to(root_dir).as_posix() for p in py_files}
            ...
            ctx.logs.append({"warning": f"Could not load existing AST: {e}"})
