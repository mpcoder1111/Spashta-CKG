...
class BuildContext:
    def __init__(self, root_path: Path, instructions: dict, mapping: dict, schema_enforcer: SchemaEnforcer):
        ...
        # CSS coverage P2 — @keyframes/animation edges (config-driven, off when 'animations' is absent).
        anim = self.mapping.get("animations")
        if isinstance(anim, dict):
            for k in ("defines_edge", "uses_edge"):
                if anim.get(k): self._allowed_mapping_edges.add(anim[k])
...
def build_for_file(css_file: Path, ctx: BuildContext):
    try:
        ...
        # Keyframes / animations (CSS coverage P2). Config-driven: `animations` present → model them as
        # Keyframes nodes (+ a normalized body_hash for identical-duplicate detection) and uses_animation
        # references; absent → the legacy 'Animations unmodeled' ambiguity. The @keyframes blocks are
        # stripped from clean_content AFTER modeling so their to/from/percent stops don't leak into
        # StyleElement in the selector pass below.
        anim_cfg = ctx.mapping.get("animations")
        if isinstance(anim_cfg, dict) and anim_cfg.get("node"):
            clean_content = _model_animations(ctx, clean_content, anim_cfg)
        elif strict_rules.get("keyframes", {}).get("never_emit_node"):
            for kf in re.finditer(r"@keyframes\s+([a-zA-Z][a-zA-Z0-9_-]*)", clean_content):
...
def _model_animations(ctx, clean_content: str, cfg: dict) -> str:
    """CSS coverage P2. Emit a Keyframes node (+ normalized body_hash) + a `defines` edge for each
    `@keyframes NAME { … }`, and a `uses_animation` edge for each `animation:`/`animation-name:` reference
    (name joined by name to the Keyframes). Returns clean_content with the @keyframes BLOCKS blanked out
    (newlines preserved) so their to/from/percent stops don't leak into StyleElement in the selector pass.
    Deterministic: a literal identifier resolves; timing keywords / units / functions are filtered out."""
    node_t, def_e, use_e = cfg["node"], cfg["defines_edge"], cfg["uses_edge"]
    stop = set(cfg.get("keyword_stopwords", []))
    spans = []
    for m in re.finditer(r"@keyframes\s+([a-zA-Z][a-zA-Z0-9_-]*)", clean_content):
        name = m.group(1)
        brace = clean_content.find("{", m.end())
        if brace == -1:
            continue
        depth, i = 0, brace
        while i < len(clean_content):
            if clean_content[i] == "{":
                ...
                if depth == 0:
                    break
            i += 1
        body = clean_content[brace + 1:i]
        body_hash = hashlib.md5(re.sub(r"\s+", "", body).encode("utf-8")).hexdigest()
        line_start = clean_content[:m.start()].count("\n") + 1
        kf_id = ctx.emit_node(node_t, name, scope=ctx.current_file, confidence="heuristic",
                              line_start=line_start, body_hash=body_hash)
        ctx.emit_edge(def_e, ctx.current_file_id, kf_id, "Stylesheet", node_t)
        spans.append((m.start(), min(i + 1, len(clean_content))))

    # Blank the @keyframes blocks (keep newlines) so the selector pass never sees to/from/percent stops.
    if spans:
        out, last = [], 0
        for s, e in spans:
            out.append(clean_content[last:s])
            out.append("\n" * clean_content[s:e].count("\n"))
            last = e
        out.append(clean_content[last:])
        clean_content = "".join(out)

    # animation: / animation-name: references → uses_animation (name joined by name).
    for m in re.finditer(r"animation(?:-name)?\s*:\s*([^;{}]+)", clean_content):
        for tok in re.split(r"[\s,]+", m.group(1).strip()):
            if not re.fullmatch(r"[a-zA-Z][a-zA-Z0-9_-]*", tok) or tok in stop:
                continue  # a duration/unit (0.6s) fails fullmatch; a timing keyword is a stopword
            uid = ctx.emit_node(node_t, tok, scope=ctx.current_file, confidence="heuristic")
            ctx.emit_edge(use_e, ctx.current_file_id, uid, "Stylesheet", node_t)
