...
• Uses declarative adapter rules only:
    ...
    – adapter agent rules
• Supports Detection Logic (9 rules):
    – inheritance_includes: Class inherits from named base (via extends edges)
    – file_path_contains: Node is in a file matching glob pattern
    – decorated_by: Node has incoming decorates edge from named decorator
    – requires_import: Node's file imports a named package
    – used_in_calls: Node has outgoing calls edge to named function
    – function_name: Node name matches exact string
    – arg_names_include: Node's signature.args contains named argument (2.1)
    – decorators: Node's signature.decorators contains named decorator (2.1)
    – has_outgoing_edge: Node has outgoing edge of specified type (2.1)
...
# Path Setup - REPO_ROOT is the Spashta-CKG folder (parent of Spashta_2.1)
...
def apply_enrichment_logic(node, edges_from, edges_to, node_map, detection_rules):
    ...
    # Rule: file_path_contains (Node is in a file matching glob X)
    if match and "file_path_contains" in detection_rules:
        ...
        # Check 1: Node's own file_path or name (for File/Template nodes that ARE the file)
        own_path = node.get("file_path") or node.get("name") or ""
        if own_path and fnmatch(own_path, glob_pat):
            found_file = True

        # Check 2: Parent file via incoming 'defines' edge (for Class/Function nodes)
        if not found_file:
    ...
    # Rule: has_outgoing_edge (Node has at least one outgoing edge of specified type)
    # Generic rule — works for any edge type (calls_api, submits_form, triggers_event, etc.)
    if match and "has_outgoing_edge" in detection_rules:
        required_edge_types = detection_rules["has_outgoing_edge"]
        found_edge = False
        for edge_type, target_id in edges_from.get(node_id, []):
    ...
    # Rule: arg_names_include (Check function signature args)
    # Reads signature.args from node metadata (populated by Python builder)
    if match and "arg_names_include" in detection_rules:
        required_args = detection_rules["arg_names_include"]
        sig = node.get("signature", {})
        node_args = sig.get("args", []) if isinstance(sig, dict) else []
        if not any(arg in node_args for arg in required_args):
            match = False

    # Rule: decorators (Check function/class signature.decorators metadata)
    # Reads signature.decorators from node metadata (populated by Python builder)
    # Matches against decorator names like "@receiver", "@login_required"
    if match and "decorators" in detection_rules:
        required_decs = detection_rules["decorators"]
        sig = node.get("signature", {})
        node_decs = sig.get("decorators", []) if isinstance(sig, dict) else []
        # Normalize: strip @ and (...) for matching
        # node_decs example: ["@receiver(...)", "@login_required"]
        normalized_decs = []
        for d in node_decs:
            clean = d.lstrip("@").split("(")[0]
            normalized_decs.append(clean)
        if not any(rd in normalized_decs for rd in required_decs):
            match = False
