...
**Usage:**
```bash
python Spashta-CKG/Spashta_2.1/runtime/query_spashta.py --help
```
...
| Command | Description |
|---------|-------------|
| `stats --json` | Graph statistics (node/edge/type counts, semantic roles) |
| `search "keyword" --json` | Find nodes by name, type (`--type`), or app (`--app`) |
| `locate "node_id" --json` | Get file path + line numbers (auto-detects File: prefix) |
| `read "node_id" --json` | Read actual source code snippet |
| `details "node_id" --json` | Full metadata: signature, decorators, docstring, roles |
| `dependencies "node_id" --json` | What does this contain/use (outgoing edges) |
| `impact "node_id" --depth 2 --json` | Who depends on this (with layer labels + coverage) |
| `call-graph "node_id" --json` | Function call chains (Python only) |
| `list-files --json` | List all indexed files (with `--app` filter) |
| `scope-check "node_id" --analyzed "a.py,b.py" --json` | Coverage self-audit |

**Example:**
```bash
python query_spashta.py search "UserView" --type Class --json
```
