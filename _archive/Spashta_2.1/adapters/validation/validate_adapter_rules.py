...
def validate_framework_mapping(mapping_data: Dict, nodes_schema: Dict) -> tuple:
    """Returns (violations: list, warnings: list)."""
    violations = []
    warnings = []
    valid_node_types = set(nodes_schema.keys())

    # Detection rules implemented in enrichment engine (enrich_runtime_ast.py)
    IMPLEMENTED_DETECTION_RULES = {
        "inheritance_includes",   # checks extends edges to named base class
        "file_path_contains",     # checks node file_path/name or parent file via defines edge
        "decorated_by",           # checks incoming decorates edges
        "requires_import",        # checks file imports edges
        "used_in_calls",          # checks outgoing calls edges
        "function_name",          # checks exact node name match
        "arg_names_include",      # checks signature.args metadata
        "decorators",             # checks signature.decorators metadata
        "has_outgoing_edge",      # checks outgoing edges of specified type
    }
    # Known but NOT yet implemented (documented as dead rules)
    KNOWN_UNIMPLEMENTED_RULES = {
        "assigned_value",         # needs builder to store RHS call name
        "context",                # needs scope-aware detection
        "contains_attributes",    # HTML-specific, replaced by has_outgoing_edge
    }
    ...
    for idx, rule in enumerate(mappings):
        ...
        # Warn (not error) if semantic_role is not a Core node type.
        sem_role = rule.get("semantic_role")
        if sem_role and sem_role not in valid_node_types:
             warnings.append({
                "file": "framework_mapping.json",
                "issue": "Custom Semantic Role",
                "key": f"mappings[{idx}].semantic_role",
                "value": sem_role,
                "details": "Role is not a Core node type — expected for framework-specific roles"
            })

        # Validate detection rule keys — catch typos and unknown rules
        detection = rule.get("detection_rules", {})
        for rule_key in detection:
            if rule_key in IMPLEMENTED_DETECTION_RULES:
                pass  # Valid and working
            elif rule_key in KNOWN_UNIMPLEMENTED_RULES:
                # Check if mapping has _known_limitation flag
                if not rule.get("_known_limitation"):
                    warnings.append({
                        "file": "framework_mapping.json",
                        "issue": "Unimplemented Detection Rule",
                        "key": f"mappings[{idx}].detection_rules.{rule_key}",
                        "value": rule_key,
                        "details": "This rule is not implemented in enrichment engine — mapping will never match. Add _known_limitation field to acknowledge."
                    })
            else:
                violations.append({
                    "file": "framework_mapping.json",
                    "issue": "Unknown Detection Rule",
                    "key": f"mappings[{idx}].detection_rules.{rule_key}",
                    "value": rule_key,
                    "details": f"Rule not recognized. Valid rules: {sorted(IMPLEMENTED_DETECTION_RULES)}"
                })
...
def validate_adapter(framework: str, output_file: str = None):
    ...
    for fname in rule_files:
        ...
        if not core_file.exists():
             report["warnings"].append({"file": fname, "issue": "Missing Core Rule File", "details": "Core file not found in core/agent_rules/ — skipped"})
             continue
    ...
    if mapping_file.exists():
        try:
            ...
            errs, warns = validate_framework_mapping(mapping_data, nodes_data)
