...
def validate_equivalence(raw_path, enriched_path):
    ...
    for node_id, raw_node in raw_node_map.items():
        ...
        # Only 'id' and 'node_type' define the structural topology.
        # Builders may use 'node_type' or 'type' — check both for compatibility.
        raw_type = raw_node.get('node_type') or raw_node.get('type')
        enriched_type = enriched_node.get('node_type') or enriched_node.get('type')
        if raw_type != enriched_type:
            violations.append({
                "type": "node_type_changed",
                "detail": f"Node {node_id} type changed from '{raw_type}' to '{enriched_type}'"
            })
    ...
    # Builders may use 'source'/'target'/'type' or 'from'/'to'/'edge' — handle both
    def get_edge_key(edge):
        src = edge.get('source', edge.get('from', ''))
        tgt = edge.get('target', edge.get('to', ''))
        typ = edge.get('type', edge.get('edge', ''))
