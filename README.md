...
Most AI coding tools rely on RAG (Vector Search) or raw file reading—methods that are often probabilistic or resource-expensive (wasting token limits). Spashta-CKG introduces a **Deterministic Architectural Memory**:
...
4.  **Enrich (First Run)**: Provide the following prompt to your AI Agent (Antigravity, Cursor, Windsurf, etc.) to start using it:
    ...
    ```prompt
    Hello. This project follows a Code Knowledge Graph (CKG)–based architecture.
