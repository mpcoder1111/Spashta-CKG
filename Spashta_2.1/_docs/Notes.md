...
The `project_root` in `Spashta_2.1/project/profile.json` determines which folder Spashta analyzes.

### Path Resolution
All relative paths are resolved from `SPASHTA_ROOT` (`Spashta_2.1` folder).

### Configuration Values

| Scenario | `project_root` Value | Explanation |
|----------|---------------------|-------------|
| **Production** (Spashta-CKG copied into user's project) | `"../.."` | Goes: `Spashta_2.1` → `Spashta-CKG` → `UserProject` |
| **Testing** (analyzing `_demo` folder) | `"../_demo"` | Goes: `Spashta_2.1` → `Spashta-CKG/_demo` |

### Folder Structure Reference

**Production (user's project):**
```
UserProject/                  ← "../.." from Spashta_2.1
├── manage.py
├── src/
└── Spashta-CKG/
    └── Spashta_2.1/          ← SPASHTA_ROOT
        └── project/
            └── profile.json
```
